...
class PPOTrain:
    def __init__(self, Policy, Old_Policy, gamma=0.95, clip_value=0.2, c_1=1, c_2=0.01):
        ...
        pi_trainable = self.Policy.get_trainable_variables() # Policy network 의 variable 을 가져움
        old_pi_trainable = self.Old_Policy.get_trainable_variables()  # Policy network 의 variable 을 가져움

        # assign_operations for policy parameter values to old policy parameters
        with tf.variable_scope('assign_op'): #policy -> old_policy로 할당
            self.assign_ops = []
        ...
        with tf.variable_scope('loss'):#loss 구하는 것
            # construct computation graph for loss_clip
            # ratios = tf.divide(act_probs, act_probs_old)
            ratios = tf.exp(tf.log(tf.clip_by_value(act_probs, 1e-10, 1.0))#(value, min, max)

    ...
    with tf.Session() as sess:
        ...
        for iteration in range(args.iteration):
            ...
            writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag='episode_reward', simple_value=sum(rewards))])
                               , iteration)
            print(sum(rewards))
            if sum(rewards) >= 195: #episode 가 끝날때 까지 돌린다
                success_num += 1
                if success_num >= 100:
            ...
            gaes = PPO.get_gaes(rewards=rewards, v_preds=v_preds, v_preds_next=v_preds_next)#advantage 계산

            # convert list to numpy array for feeding tf.placeholder
            observations = np.reshape(observations, newshape=[-1] + list(ob_space.shape))
            actions = np.array(actions).astype(dtype=np.int32)
            gaes = np.array(gaes).astype(dtype=np.float32)
            gaes = (gaes - gaes.mean()) / gaes.std() # 표준화 advantage 값들을 공정하게 비교가(정규분포를 따를 평균이 0, 표준편차 1인 표준정규 분포를 따르는)
            rewards = np.array(rewards).astype(dtype=np.float32)
            v_preds_next = np.array(v_preds_next).astype(dtype=np.float32)

            PPO.assign_policy_parameters()#현재 네트워크를 이전 old network에 복

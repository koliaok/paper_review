...
class Discriminator:
    def __init__(self, env):
        ...
        with tf.variable_scope('discriminator'):
            ...
            self.expert_s = tf.placeholder(dtype=tf.float32, shape=[None] + list(env.observation_space.shape)) #expert의 state 값(PPO 만든 Observation Sample)
            self.expert_a = tf.placeholder(dtype=tf.int32, shape=[None]) #expert action(PPO가 각 Opservation에서 한 Action)
            ...
            with tf.variable_scope('loss'):
                ...
                loss = loss_expert + loss_agent# loss 최대화
                loss = -loss
    ...
    def construct_network(self, input):
        ...
        prob = tf.layers.dense(inputs=layer_3, units=1, activation=tf.sigmoid, name='prob') #sigmoid (0, 1)
        return prob
